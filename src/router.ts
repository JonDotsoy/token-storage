...
// OAuth Clients
router.route("PUT", "/oauth_clients/:client_id", async (req) => {
...
router.route("GET", "/oauth_clients/:client_id", async (req) => {
  const { client_id } = params(req);
  if (!client_id) {
    return new Response("Missing client_id", { status: 400 });
  }
...
router.route("DELETE", "/oauth_clients/:client_id", async (req) => {
  const { client_id } = params(req);
  if (!client_id) {
    return new Response("Missing client_id", { status: 400 });
  }
...
router.route("GET", "/oauth_clients", async (req) => {
  return await getOAuthClients();
});

// Connections
router.route("PUT", "/connections/:connection_id", async (req) => {
...
router.route("GET", "/connections/:connection_id", async (req) => {
  const { connection_id } = params(req);
  if (!connection_id) {
    return new Response("Missing connection_id", { status: 400 });
  }
...
router.route("DELETE", "/connections/:connection_id", async (req) => {
  const { connection_id } = params(req);
  if (!connection_id) {
    return new Response("Missing connection_id", { status: 400 });
  }
...
router.route("GET", "/connections", async (req) => {
  return await getConnections();
});

// Auth URL
router.route("GET", "/connections/:connection_id/auth_url", async (req) => {
  const { connection_id } = params(req);
  if (!connection_id) {
    return new Response("Missing connection_id", { status: 400 });
  }
  ...
  return { auth_url: authUrl.toString() };
});

// Authorizations
router.route("POST", "/connections/:connection_id/exchange", async (req) => {
  const { connection_id } = params(req);
  if (!connection_id) {
    return new Response("Missing connection_id", { status: 400 });
  }
...
router.route("GET", "/authorizations/:authorization_id/token", async (req) => {
