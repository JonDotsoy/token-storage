...
describe("Migration", () => {
    ...
    beforeEach(async () => {
        ...
        const createTableIfNotExistsMigrations = async () => await connection.run(`
    ...
    test("should apply a single migration and provide utility functions", async () => {
                ...
                up(connection) {
                    return {
                        async run() {
                            ...
                            await connection.run("CREATE TABLE users (uid INTEGER PRIMARY KEY, name VARCHAR, email VARCHAR)")
                        },
                        utils: {
                            async getUsers() {
                                const result = await connection.runAndReadAll("SELECT * FROM users")
                                return result.getRowObjectsJS() as { uid: number, name: string, email: string }[]
                            },
                            async createUser(user: { uid: number, name: string, email: string }) {
                                await connection.run("INSERT INTO users (uid, name, email) VALUES (?, ?, ?)", [user.uid, user.name, user.email])
                            }
                        }
                    }
                }
        ...
        expect(users).toEqual([{ uid: 1, name: "John", email: "john@example.com" }]);
    })
    ...
    test("should apply multiple migrations in sequence and merge utility functions", async () => {
                ...
                up(connection) {
                    return {
                        run: async () => {
                            ...
                            await connection.run("CREATE TABLE products (uid INTEGER PRIMARY KEY, name VARCHAR, price INTEGER, user_id INTEGER)")
                        },
                        utils: {
                            async getProducts() {
                                const result = await connection.runAndReadAll("SELECT * FROM products")
                                return result.getRowObjectsJS() as { uid: number, name: string, price: number, user_id: number }[]
                            },
                            async createProduct(product: { uid: number, name: string, price: number, user_id: number }) {
                                await connection.run("INSERT INTO products (uid, name, price, user_id) VALUES (?, ?, ?, ?)", [product.uid, product.name, product.price, product.user_id])
                            }
                        }
                    }
                }
        ...
        await utils.createUser({ uid: 1, name: "John", email: "john@example.com" })
        await utils.createProduct({ uid: 1, name: "Product 1", price: 100, user_id: 1 })
        await utils.createProduct({ uid: 2, name: "Product 2", price: 200, user_id: 1 })
